/*
  # Sistema Chat Completo per SardAI

  1. Nuove Tabelle
    - `user_subscriptions` - Gestione abbonamenti e periodi di prova
    - `chat_sessions` - Sessioni di chat degli utenti
    - `chat_messages` - Messaggi delle chat

  2. Funzioni
    - `get_user_plan_status()` - Ottiene stato piano utente
    - `create_chat_session()` - Crea nuova sessione chat
    - `add_chat_message()` - Aggiunge messaggio alla chat

  3. Sicurezza
    - RLS abilitato su tutte le tabelle
    - Policies per controllo accesso
    - Validazione accesso premium
*/

-- Drop existing functions that might have different return types
DROP FUNCTION IF EXISTS get_user_plan_status(uuid);
DROP FUNCTION IF EXISTS create_chat_session(text, text);
DROP FUNCTION IF EXISTS add_chat_message(uuid, text, text, integer);
...
CREATE FUNCTION get_user_plan_status(user_uuid uuid DEFAULT auth.uid())
RETURNS TABLE(
  plan text,
  status text,
...
CREATE FUNCTION create_chat_session(
...
CREATE FUNCTION add_chat_message(
  session_uuid uuid,
  message_role text,
